...
export const RecordView: React.FC<RecordViewProps> = ({
  ...
  return (
    <div className="flex h-full bg-white">
      {/* Left: Timeline */}
      <div className="flex-1 overflow-y-auto relative bg-white h-full pb-20 border-r border-stone-100">
        <div className="sticky top-0 bg-white/90 backdrop-blur-md z-20 px-4 py-3 flex justify-between items-center border-b border-stone-50">
             <span className="text-xs font-bold text-secondary tracking-wider uppercase">真实记录</span>
             {selectedTaskId && <span className="text-[10px] text-secondary font-medium bg-secondary/5 px-2 py-1 rounded-full">记录模式</span>}
        </div>
      ...
      {/* Right: Task List */}
      <div className="w-[150px] bg-[#fafaf9] flex flex-col h-full">
        <div className="p-4 z-10 sticky top-0 bg-[#fafaf9]/95 backdrop-blur border-b border-stone-50">
          <h2 className="text-[10px] font-bold text-stone-400 uppercase tracking-widest ml-1">任务选择</h2>
        </div>
